function [fig_handle, axes_handle, scroll_bar_handles, scroll_func] = ...
	videofig(shankStr,currentComposition,num_frames, redraw_func, play_fps, big_scroll, ...
	key_func,varargin)
...
%   Jo�o Filipe Henriques, 2010
	...
	if nargin < 5 || isempty(play_fps), play_fps = 25; end  %play speed (frames per second)
	if nargin < 6 || isempty(big_scroll), big_scroll = 30; end  %page-up and page-down advance, in frames
	if nargin < 7, key_func = []; end
	...
	%initialize figure
	fig_handle = figure('Position',[304         144        1500         600],'Color',[.3 .3 .3], 'MenuBar','none', 'Units','norm', ...
	...
	function scroll(new_f)
		...
		redraw_func(f,currentComposition,shankStr);
