...
%   License: BSD 3 clause
    ...
    properties(Constant)
        ...
        GOOGLE_CLOUD_ROOT='https://storage.googleapis.com/cytogenie.app';
    ...
    methods(Static)
        ...
        %GetHosts looks up a list of URL root "mirrors" expected to 
        %contain the folder of urlPath.  The internal list is used 
        %if this file is not found <HOME>/.herzenbergLabHosts.properties
        %If on the other hand the properties file is found then the 
        %first server looked up is indicated by the property host.1 and 
        %if this server is unavailable the URL root in host.2 is used.
        %The next server at host.3 is checked if host.2 is also 
        %unavailable and the lookup stops when # of hosts specified
        %by host.count is checked.
        ...
        function [hosts, googleDir, m]=GetHosts(urlPath)
            ...
            if ~googleDriveOnly
                ...
                m0=m.addIfMissing(WebDownload.PROPERTY_HOST, WebDownload.GOOGLE_CLOUD_ROOT);
            ...
            if ~googleDriveOnly  && ~isequal(hosts{1}, WebDownload.GOOGLE_CLOUD_ROOT)
                app=BasicMap.Global;
                if ~app.is(WebDownload.PROPERTY_IS_FIRST_GOOGLE_CLOUD)
                    %prioritize Google Cloud in local properties if first time
                    hosts=StringArray.Sort(hosts, {WebDownload.GOOGLE_CLOUD_ROOT});
        ...
        function [ok, cancelled, dwl, dlg, notExistsFiles,...
            ...
            [cancelled, ~, dwl, dlg]=...
                ...
                waitWhenDone, allowCancel, where);
            if nargout>4 %CM: Clause rewritten on 2022/03/09
                files = cell(1,N);
                exists = false(1,N);
        ...
        function  file=LocateUri(uri, downloadsFolder)
            %Example
            %SuhFcs.Locate('https://storage.googleapis.com/cytogenie.app/GetDown2/domains/FACS/demo/bCellMacrophageDiscovery/all_3-3.fcs')
            ...
            if download
                [host,path]=WebDownload.UrlParts(uri);
                if ispc
                    path=strrep(path, '/', '\');
                end
                ...
                if isempty(downloadsFolder)
                    ...
                    file=fullfile(downloadsFolder, f);
                    if exist(file, 'file') 
                        return;
                    end
                end
