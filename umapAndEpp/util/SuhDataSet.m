...
classdef SuhDataSet < handle
    ...
    methods
        ...
        function done=finalizeSubset(this, subset, id)
            done=sum(subset.selected);
            if any(this.finalSubsetIds(subset.selected)~=0)
                prior=sum(this.finalSubsetIds(subset.selected)~=0);
                if prior~=done
                    prefix='YIKES!!  ';
                else
                    prefix='';
                end
                warning('%s%d/%d overlap leaf #%d by %d other leaves', ...
                    prefix, prior, done, id ,...
                    length(unique(this.finalSubsetIds(subset.selected))))
                done=done-prior;
            end
        ...
        function [sLbls,mx]=getTestSetLabels(this)
            mx=max(this.labels);
            if any(this.finalSubsetIds==0)
                l0=this.finalSubsetIds==0;
        ...
        function [qf, matchTable]=characterize(this, epp, pu, visible)
            ...
            matchStrategy=2; % match by QFMatch accelerated by overlap
            [tNames, clrs, tLbls]=LabelBasics.GetNamesColorsInLabelOrder(...
                this.labels, this.lblMap);
            [sLbls,mx]=this.getTestSetLabels;
            [eppLeafNames, missingNames]=epp.getLeafNames;
            if ~isempty(missingNames)
                eppLeafNames(missingNames)=[];
            end
