...
%   License: BSD 3 clause
    ...
    methods(Static)
        ...
        function c=Concordance(tData, sData, tIdPerRow,tIdSet, sIdPerRow, ...
                sIdSet, areEqual)
            c=-1;
            if isempty(tData)||isempty(sData)
                return;
            ...
            sizeTrainingSet=sum(tChoices);
            sizeTestSet=sum(sChoices);
            c=Clusters.Concordance(sizeRecalled, sizeTrainingSet, sizeTestSet);
        end

        function c=RobustConcordance(tData, sData, tIdPerRow,tIdSet, sIdPerRow, ...
                sIdSet, areEqual, outlierFraction)
            c=-1;
            if isempty(tData)||isempty(sData)
                return;
            end
            ...
            if size(tData,1) ~= size(tIdPerRow,1) || size(sData,1) ~= size(sIdPerRow,1)
                warning('# of classification IDs must equal # of data points');
                return;
            end
            try
                tChoices=MatBasics.LookForIds(tIdPerRow, tIdSet);
                [~,~,~,tOutliers] = robustcov(tData(tChoices,:),'OutlierFraction',outlierFraction);
                tChoices(tChoices) = ~tOutliers;
                
                sChoices=MatBasics.LookForIds(sIdPerRow, sIdSet);
                [~,~,~,sOutliers] = robustcov(sData(sChoices,:),'OutlierFraction',outlierFraction);
                sChoices(sChoices) = ~sOutliers;
                ...
                % most frequent exception is
                % "The sample size should be at least twice the number of
                %       variables."
