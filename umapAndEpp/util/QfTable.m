...
classdef QfTable < handle
    ...
    methods(Static)
        ...
        function [data, names, widths, tips, unmatched, plotIdx, freqIdx,...
                ...
                robustConcIdx]=Contents(qf, clrs, pu)            
            numIdx=1;
            nameIdx=2;
            matchesIdx=3;
            similarityIdx=4;
            ...
            try
                ...
                hasConcordance=size(data,2)>11;
            end
            if hasConcordance %includes concordance
                idW=widths(end,:);
                widths(end,:)=[4 -1];
                idN=names{end};
                names{end}=Html.WrapSm('Concordance');
                idTip=tips{end};
                tips{end}='Concordance measure';
                idIdx=13;
                concIdx=11;
                robustConcIdx=12;
                widths(end+1,:)=[4 -1];
                names{end+1}=Html.WrapSm('Robust<br>concordance');
                tips{end+1}='Robust concordance measure';
                widths(end+1, :)=idW;
                names{end+1}=idN;
                tips{end+1}=idTip;
            end
